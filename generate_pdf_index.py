...
def generate_pdf_index():
    ...
    pdf_files = [ i[2:] for i in pdf_files ]
...
{pdf_items}
    ...
    if pdf_files:
        for pdf_file in pdf_files:
            try:
                # Get file size
                file_size = os.path.getsize(os.path.join("./", pdf_file))
...
"""
        shutil.copyfile(os.path.join("./", pdf_file), os.path.join("docs", pdf_file))
    else:
    ...
    if pdf_files:
        ...
        for pdf in pdf_files:
            size = format_file_size(os.path.getsize(os.path.join("./", pdf)))
            print(f"   • {pdf} ({size})")
    else:
